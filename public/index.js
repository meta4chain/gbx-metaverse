    ...
    loader.load(GLTFName, (gltf) => {
        ...
        if(collider){
            ...
            for(let j=0; j < gltf.scene.children.length; j++){
                if(gltf.scene.children[j].name !=  "Puffs" && gltf.scene.children[j].name !=  "Cadeiras_Mesinhas" && gltf.scene.children[j].name !=  "Rack_Tv" && gltf.scene.children[j].name !=  "Estantes"){
                    // console.log("valor: " + j + " name: " + gltf.scene.children[j].name)
                    worldOctree.fromGraphNode(gltf.scene.children[j]);
                }
                if(gltf.scene.children[j].name == "Vidro_Janela"){
                    gltf.scene.children[j].material.opacity = 0.1;
...
function onStart(level) {
    ...
    let path = '/Avatar/flaviohello.glb'
    ...
    characterThiago.loadGLTF()
      ...
      loaderSphere.load('./files/GBXVesting/sphere.jpg', function ( texture ) {
...
function createScene(level, loader) {
    ...
    if (level == "gold") {
        let collider = true;
        loaderGLTF(loader, "Ouro/Ouro03.glb", { x: 0, y: 2, z: 0 }, 1, collider)
